...
Функция ИнициализироватьГенераторы(Репо, Проекты)
	...
	Для Каждого Проект Из Проекты Цикл
		...
		Лог.Отладка("Проект %1", Проект);
		Пути = СтрРазделить(СтрЗаменить(Проект, "\", "/"), "/");

		ИмяПроекта = Пути[Пути.ВГраница()];
		Лог.Отладка("ИмяПроекта %1", ИмяПроекта);

		Путь = ОбъединитьПути(Репо, Проект);
		Лог.Отладка("Для проекта %1 использован путь %2", ИмяПроекта, Путь);
		Генераторы.Вставить(ИмяПроекта, Новый Путь1СПоМетаданным(Путь));
	КонецЦикла;
